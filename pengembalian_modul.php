...
if (empty($_GET['aksi'])){ 
  $no=1;
  $lihat=$mysqli->query("SELECT * FROM pengembalian Order By kode_pinjam");
...
if (isset($_GET['opsi']) && $_GET['opsi']=='status'){
 $status=$mysqli->query("SELECT * FROM pengembalian WHERE kode_pinjam='$_GET[id]'");
 while ($sts=$status->fetch_array()){ ?>
 <form action="?halaman=pengembalian&skrip
 =status" method="post">
