...
if (empty($_GET['aksi'])){ 
  $no = 1;
  $lihat = $mysqli->query("SELECT * FROM penerbit Order By kode_penerbit");
...
?>
  ...
  <?php while ($l = $lihat->fetch_array()){ ?>    
   <tr class="data">
    ...
    <a href="?halaman=penerbit&skrip=hapus&id=<?php echo "$l[kode_penerbit]";?>" onClick="return hapus();">
    Hapus</a>
    </td>
   </tr>
   <?php $no++; } ?>
...
if (isset($_GET['opsi']) && $_GET['opsi'] == 'ubah'){
 $ubah = $mysqli->query("SELECT * FROM penerbit WHERE kode_penerbit='$_GET[id]'");
...
<?php
// Form Untuk Mengekspor DATA PENERBIT
