...
if (empty($_GET['aksi'])){ 
  $no=1;
  $lihat=$mysqli->query("SELECT * FROM anggota ORDER BY kode_anggota");
...
?>
  ...
  <?php while ($l = $lihat->fetch_array()){ ?>	
   <tr class="data">
	...
	<a href="?halaman=anggota&opsi=ubah&id=<?php echo $l['kode_anggota']; ?>">Ubah</a> | 
	<img src="ikon/hapus.png" width="20" height="20" align="absmiddle" /> 
	<a href="?halaman=anggota&skrip=hapus&id=<?php echo $l['kode_anggota'];?>" onClick="return hapus();">Hapus</a>
	</td>
   </tr>
   <?php $no++; } ?>
  </table>
...
if (isset($_GET['opsi']) && $_GET['opsi'] == 'tambah'){ ?>
 <form action="?halaman=anggota&skrip=tambah" method="post" name="tambah_anggota">
  ...
  <iframe width=174 height=189 name="gToday:normal:kalender/agenda.js" id="gToday:normal:kalender/agenda.js" src="kalender/ipopeng.htm" scrolling="no" frameborder="0" style="visibility:visible; z-index:999; position:absolute; top:-500px; left:-500px;"></iframe>
</form>
<?php } ?>
...
if (isset($_GET['opsi']) && $_GET['opsi'] == 'ubah'){
 $ubah = $mysqli->query("SELECT * FROM anggota WHERE kode_anggota='$_GET[id]'");
...
if (isset($_GET['opsi']) && $_GET['opsi'] == 'ekspor'){ ?>
